...
export default function HomePage() {
  ...
  const recentArticles: ArticleForDisplay[] = result?.articles.map(article => ({
    id: article._id,
    slug: article.slug,
    title: article.title,
        ...
        <div className="pt-24 pb-8">
          <div className="max-w-6xl mx-auto px-4">
            <div className="mb-8">
              ...
              <p className="text-gray-600">Ready to read or write your next story?</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-12">
              ...
              <Link href="/articles" className="group p-6 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div className="flex items-center mb-4">
                  <div className="p-3 bg-green-100 rounded-lg group-hover:bg-green-200 transition-colors">
                    <BookOpen className="w-6 h-6 text-green-600" />
                  </div>
                  <h3 className="text-lg font-semibold ml-3">Browse Articles</h3>
                </div>
                <p className="text-gray-600">Discover stories and tip the writers you love</p>
              </Link>

              {hasWallet ? (
                <Link href={`/${user.username}?tab=earnings`} className="group p-6 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                  <div className="flex items-center mb-4">
                    <div className="p-3 bg-purple-100 rounded-lg group-hover:bg-purple-200 transition-colors">
                      <TrendingUp className="w-6 h-6 text-purple-600" />
                    </div>
                    <h3 className="text-lg font-semibold ml-3">Your Earnings</h3>
                  </div>
                  <p className="text-gray-600">Track tips received and article performance</p>
                </Link>
              ) : (
                <Link href="/guide" className="group p-6 bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                  <div className="flex items-center mb-4">
                    <div className="p-3 bg-amber-100 rounded-lg group-hover:bg-amber-200 transition-colors">
                      <Wallet className="w-6 h-6 text-amber-600" />
                    </div>
                    <h3 className="text-lg font-semibold ml-3">Set Up Wallet</h3>
                  </div>
                  <p className="text-gray-600">Connect a Stellar wallet to send and receive tips</p>
                </Link>
              )}
            </div>
